...
if ($job != ''){
  ...
  if ($job == 'get_companies'){
    ...
    // Get companies
    $query = "SELECT * FROM tb_evaluaciones_proyectos LEFT JOIN tb_proyectos ON 
    		tb_evaluaciones_proyectos.proyecto_evaluado = tb_proyectos.id_proyecto ORDER BY fecha_evaluacion DESC
    		";
    ...
    if (!$query){
      ...
      while ($company = mysqli_fetch_array($query)){
        ...
        $functions .= '<li class="function_edit"><a data-id="'   . $company['id_revisiones_proyectos'] . '" data-name="' . $company['nombre_proyecto'] . '"><span>Edit</span></a></li>';
        $functions .= '<li class="function_delete"><a data-id="' . $company['id_revisiones_proyectos'] . '" data-name="' . $company['nombre_proyecto'] . '"><span>Delete</span></a></li>';
        ...
        if ($company['estado_evaluacion'] ==1){
        	$estado = $lang['EVALUATED'];
        }
        if ($company['estado_evaluacion'] == 0){
        	$estado = $lang['NOT_EVALUATED'];
        }
        $revision = get_nombre_revision($company['revision_evaluada']);
        $proyecto_nombre = get_proyecto($company['proyecto_evaluado']);
        $equipo_proyecto = get_equipo_proyecto($company['proyecto_evaluado']);
        $team = get_team($equipo_proyecto);
        $evaluador = get_nombre( $company['evaluador_proyecto']);
        ...
        $mysql_data[] = array(
          ...
          "revision"    => $revision,
        		...
        		"fecha"    => $company['fecha_evaluacion'],
        		"puntos"    => $company['puntos_obtenidos'],
        		"comentarios" => $company['comentarios_evaluados']
