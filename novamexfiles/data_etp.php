...
if ($job != ''){
  ...
  if ($job == 'get_companies'){
    ...
    // Get companies
    $query = "SELECT * FROM tb_revisiones_tareas_proactividad LEFT JOIN tb_tareas_proactividad ON 
    		tb_revisiones_tareas_proactividad.tareas_proactividad_revisado = tb_tareas_proactividad.id_tareas_proactividad ORDER BY fecha_evaluacion DESC
    		";
    ...
    if (!$query){
      ...
      while ($company = mysqli_fetch_array($query)){
        ...
        $functions .= '<li class="function_edit"><a data-id="'   . $company['id_tareas_proactividad'] . '" data-name="' . $company['id_revisiones_rci'] . '"><span>Edit</span></a></li>';
        $functions .= '<li class="function_delete"><a data-id="' . $company['id_tareas_proactividad'] . '" data-name="' . $company['id_revisiones_rci'] . '"><span>Delete</span></a></li>';
        ...
        $proveedor = get_nombre($company['proveedor_tareas_proactividad']);
        $cliente = get_nombre($company['cliente_tareas_proactividad']);
        $puntos = get_puntos_etp_rev($company['tareas_proactividad_revisado']);
        
        $mysql_data[] = array(
         
          "estado"  => $estado,
          "revision"    => $company['nombre_revision'],
        		"titulo"    => $company['titulo_tareas_proactividad'],
        		"proveedor"    => $proveedor,
        		"cliente"    => $cliente,
