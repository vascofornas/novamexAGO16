...
if ($job != ''){
  ...
  if ($job == 'get_companies'){
    ...
    // Get companies
    $query = "SELECT * FROM tb_revisiones_rci LEFT JOIN tb_requerimientos_cliente_interno ON 
    		tb_revisiones_rci.rci_revisado = tb_requerimientos_cliente_interno.id_req_interno ORDER BY fecha_evaluacion DESC
    		";
    ...
    if (!$query){
      ...
      while ($company = mysqli_fetch_array($query)){
        ...
        $functions .= '<li class="function_edit"><a data-id="'   . $company['id_revisiones_rci'] . '" data-name="' . $company['id_revisiones_rci'] . '"><span>Edit</span></a></li>';
        $functions .= '<li class="function_delete"><a data-id="' . $company['id_revisiones_rci'] . '" data-name="' . $company['id_revisiones_rci'] . '"><span>Delete</span></a></li>';
        ...
        $proveedor = get_nombre($company['proveedor_rci']);
        $cliente = get_nombre($company['cliente_rci']);
        $puntos = get_puntos_epi_rev($company['rci_revisado']);
        
        $mysql_data[] = array(
         
          "estado"  => $estado,
          "revision"    => $company['nombre_revision'],
        		"rci"    => $company['titulo_rci'],
        		"proveedor"    => $proveedor,
        		"cliente"    => $cliente,
