$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    "ajax": "data_proyectos.php?job=get_companies",
    "columns": [
      
      { "data": "nombre_proyecto",   "sClass": "company_name" },
    ...
    "oLanguage": {
      ...
      "sInfo":          "Total of _TOTAL_ projects (showing from _START_ to _END_)",
  ...
  $(document).on('click', '#add_company', function(e){
    e.preventDefault();
    $('.lightbox_content h2').text('Add Project');
    $('#form_company button').text('Add Project');
    $('#form_company').attr('class', 'form add');
    $('#form_company').attr('data-id', '');
    $('#form_company .field_container label.error').hide();
    $('#form_company .field_container').removeClass('valid').removeClass('error');
   
    $('#form_company #nombre_proyecto').val('');
    $('#form_company #descripcion_proyecto').val('');
    $('#form_company #tipo_proyecto').val('');
    $('#form_company #equipo_proyecto').val('');
    $('#form_company #evaluador_proyecto').val('');
  ...
  $(document).on('submit', '#form_company.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'data_proyectos.php?job=add_company',
        cache:        false,
        data:         form_data,
        dataType:     'json',
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            ...
            show_message("Project with Title '" + company_name + "' added sucessfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
