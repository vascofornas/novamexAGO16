$(document).ready(function(){
  ...
  var table_companies = $('#table_companies').dataTable({
    "ajax": "data_free_points.php?job=get_companies",
    "columns": [
      
      { "data": "level5_user",   "sClass": "company_name" },
     
      { "data": "level3_user", },
      { "data": "total_puntos_libres", },
      { "data": "max_puntos_libres", },
  ...
  $(document).on('submit', '#form_company.add', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var request   = $.ajax({
        url:          'data_free_points.php?job=add_company',
        cache:        false,
        data:         form_data,
        dataType:     'json',
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            ...
            var company_name = $('#level5_user').val();
  ...
  $(document).on('click', '.function_edit a', function(e){
    ...
    var id      = $(this).data('id');
    var request = $.ajax({
      url:          'data_free_points.php?job=get_company',
      cache:        false,
      data:         'id=' + id,
      dataType:     'json',
      contentType:  'application/json; charset=utf-8',
    ...
    request.done(function(output){
      if (output.result == 'success'){
        ...
        $('#form_company #level5_user').val(output.data[0].level5_user);
        
        $('#form_company #level3_user').val(output.data[0].level3_user);
  ...
  $(document).on('submit', '#form_company.edit', function(e){
    ...
    if (form_company.valid() == true){
      ...
      var form_data = $('#form_company').serialize();
      var request   = $.ajax({
        url:          'data_free_points.php?job=edit_company&id=' + id,
        cache:        false,
        data:         form_data,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
      ...
      request.done(function(output){
        if (output.result == 'success'){
          ...
          table_companies.api().ajax.reload(function(){
            hide_loading_message();
            var company_name = $('#level5_user').val();
            show_message("Free points from '" + company_name + "' edited succesfully.", 'success');
          }, true);
        } else {
          hide_loading_message();
  ...
  $(document).on('click', '.function_delete a', function(e){
    ...
    if (confirm("Do yo want to delete Free Points from '" + company_name + "'?")){
      ...
      var id      = $(this).data('id');
      var request = $.ajax({
        url:          'data_free_points.php?job=delete_company&id=' + id,
        cache:        false,
        dataType:     'json',
        contentType:  'application/json; charset=utf-8',
        type:         'get'
