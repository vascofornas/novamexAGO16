...
<div class = "container">
  ...
  <HR>
  <h3><center><?php echo $lang['EVALUACION_PROVEEDOR_INTERNO']?></center></h3>
  <h4>
<?php

//run the query
$loop = mysqli_query($conexion, "SELECT * FROM tb_revisiones_rci WHERE proveedor_rci = '".$_SESSION['userSession']."'")
...
while ($row_proyectos = mysqli_fetch_array($loop))
{
		...
		$suma = $row_proyectos['evaluacion_c1'] + $row_proyectos['evaluacion_c2']+ $row_proyectos['evaluacion_c3']+ $row_proyectos['evaluacion_c4'];
			...
			?>
					<tr>
			<td style="background-color:GreenYellow ;" ><?php echo $row_proyectos['fecha_inicio_rci']?></td>
			<td style="background-color:GreenYellow ;"><?php echo get_nombre($row_proyectos['cliente_rci'])?></td>
			<td style="background-color:GreenYellow ;"><?php echo $row_proyectos['nombre_revision']?></td>
			<td style="background-color:GreenYellow ;"><?php echo $row_proyectos['titulo_rci']?></td>
